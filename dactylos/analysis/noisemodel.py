...
def fit_noisemodel(xs, ys, ys_err,  ch, detid, plotdir='.'):
    """
    ...
    noisemodel.add_data(ys, xs=xs/1000, create_distribution=False)
    ...
    ax.plot(noisemodel.xs, noise_model(noisemodel.xs, *noisemodel.best_fit_params), color='r', lw=2)
    ax.errorbar(xs/1000, ys, yerr=ys_err/2, \
                marker='o', mfc='k', mec='k', ms=5, \
                #fmt='none',\
                fmt='o', \
                ecolor='k')
