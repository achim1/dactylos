...
int main(int argc, char* argv[])
{
    ...
    parser->AddProgramDescription(description);
    ...
    *  Acquisition parameters    *
    ...
    log_info("Writing to root file " << outfile);
    ...
    log_info("ROC FPGA Release is " << board_info.ROC_FirmwareRel);
    log_info("AMC FPGA Release is " << board_info.AMC_FirmwareRel);
    std::vector<int> temps = digitizer.get_temperatures();
    for (uint ch=0; ch<digitizer.get_nchannels(); ch++)
        {
            log_info("Ch " << ch << " ADC temperature: " <<  temps[ch]);
        }
    ...
    if (run_time > 0)
      {
        log_info("Will acquire data for " << run_time << " seconds");
        long timeDelta = 0;
    ...
    log_info( acquired_events_display);
    output->cd();
    for (auto h : histos)
        {h->Write();}
    output->Write();
